...
-- dave note: what the fuck???
-- christ almighty.
-- edits:
--[[

1. fixed indentation
2. its not "if TabData == false then" its "if TabData.Locked == false then"
   - u cant compare a tbl to a bool
3. on close, callback function destroys ui immediately; script tried to closeModal() when ModalInstance has already been deleted
4. i understand the usage of chatgpt to speed shit up but COME ON he doesnt know how to evade detection 😭 pls be mindful of that
	...
	for key, value in pairs(Config) do
		if self.theme[key] ~= nil then
			if typeof(self.theme[key]) == typeof(value) then
				...
				--warn(("[UpdateTheme] Type mismatch for key '%s' (expected %s, got %s)"):format(key, typeof(self.theme[key]), typeof(value)))
			end
		else
...
function syde:Modal(Modal)
	task.spawn(function()
		...
		local function closeModal()
			...
			if not ModalInstance or not ModalInstance.Parent or not ModalInstance:FindFirstChild("Buttons") then
		...
		ModalInstance.Buttons.Confirm.MouseButton1Click:Connect(function()
			...
			if activeModals == 0 then
				if Library and Library:FindFirstChild("lib") and Library.lib:FindFirstChild("dim") then
					tweenservice:Create(Library.lib.dim, TweenInfo.new(0.73, Enum.EasingStyle.Exponential), {
	...
	function syde:Load(Config)
		...
		local function load()
			...
			if Config.AutoJoinDiscord and Config.AutoJoinDiscord.Enabled then
				...
				-- man FUCK you for automatically joinign discord serrvbers.
			end
		...
		while TimeTillLoad > 0 do
			...
			task.wait(0.01)
			TimeTillLoad = TimeTillLoad - 0.01
		end
...
function syde:Init(library)
	...
	WINDOW.resize.MouseEnter:Connect(function() animateResizeButton(true)
	end)
	WINDOW.resize.MouseLeave:Connect(function() animateResizeButton(false)
	end)
			...
			function element:ColorPicker(ColorPicker, Parent)
				...
				local function updateRecentLayout()
					...
					for _, frame in ipairs(children) do
						...
						startX = startX + frameSize + spacing
					end
				end
...
--[[	local uptimeParagraph = element:Paragraph({
	...
	function ldata:InitTab(tab)
		...
		Tab.interact.MouseButton1Click:Connect(function()
			if ldata.first == TabData.Title then
				return
			end
		...
		if lockedframe then
			...
			keyInputBox.FocusLost:Connect(function(enterPressed)
				...
				if not enterPressed or not currentLockedTabData then
					return
				end
		...
		function InitElement:EnchancedView(View)
			...
			task.spawn(function()
				while EnchancedView.Parent do
					currentRotationX = currentRotationX + (targetRotationX - currentRotationX) * 0.15
					currentRotationY = currentRotationY + (targetRotationY - currentRotationY) * 0.15
